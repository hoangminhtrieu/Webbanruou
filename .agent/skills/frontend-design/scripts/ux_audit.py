...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # --- 1. PSYCHOLOGY LAWS ---
        # Hick's Law
        matches = re.findall(r'<NavLink\b|<Link\b|<a\s+href|\bnav-item\b', content, re.IGNORECASE)
        # Filter out <link tags (case-insensitive check)
        nav_items = len([m for m in matches if not m.lower().startswith('<link')])
        if nav_items > 7:

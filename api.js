...
async function apiFetch(endpoint, options = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        data.error || data.errors?.[0]?.msg || "Lỗi không xác định",
      );
    }
    return data;
  } catch (err) {
    if (err instanceof TypeError)
      throw new Error(
        "Không thể kết nối tới server. Vui lòng chạy: npm run dev trong thư mục server/",
      );
    throw err;
  }
}
...
const API = {
  ...
  products: {
    ...
    getFilterCounts: () => apiFetch("/products/filter-counts"),
  ...
  cart: {
    ...
    update: (product_id, qty) =>
      apiFetch(`/cart/${product_id}`, {
        method: "PUT",
        body: JSON.stringify({ qty }),
      }),
    remove: (product_id) =>
      apiFetch(`/cart/${product_id}`, { method: "DELETE" }),
  ...
  orders: {
    ...
    adminAll: (params = {}) =>
      apiFetch("/orders/admin/all?" + new URLSearchParams(params)),
  },

  // ─── PAYMENTS ───────────────────────────────────
  payments: {
    vnpay: (order_id) =>
      apiFetch("/payments/vnpay/create", {
        method: "POST",
        body: JSON.stringify({ order_id }),
      }),
    momo: (order_id) =>
      apiFetch("/payments/momo/create", {
        method: "POST",
        body: JSON.stringify({ order_id }),
      }),
    stripe: (order_id) =>
      apiFetch("/payments/stripe/create-intent", {
        method: "POST",
        body: JSON.stringify({ order_id }),
      }),
  ...
  users: {
    ...
    removeWishlist: (id) =>
      apiFetch(`/users/wishlist/${id}`, { method: "DELETE" }),
    addReview: (data) =>
...
function getAuthUser() {
  try {
    return JSON.parse(localStorage.getItem("vinova_user"));
  } catch {
    return null;
  }
}
...
function isLoggedIn() {
  return !!localStorage.getItem("vinova_token");
}
...
window.doRegister = async function () {
  ...
  const full_name =
    `${nameInputs?.[0]?.value || ""} ${nameInputs?.[1]?.value || ""}`.trim();

...
const User = {
  ...
  findById(id) {
    ...
    return db
      .prepare(
        "SELECT id, email, full_name, phone, role, tier, points, is_active, created_at FROM users WHERE id = ?",
      )
      .get(id);
  },
  ...
  addPoints(id, points) {
    ...
    // Lấy tổng điểm mới để cập nhật tier
    const user = db.prepare("SELECT points FROM users WHERE id = ?").get(id);
    if (user) {
      let newTier = "silver";
      if (user.points > 5000) newTier = "platinum";
      else if (user.points > 1000) newTier = "gold";

      db.prepare("UPDATE users SET tier = ? WHERE id = ?").run(newTier, id);
    }
    return true;
  },

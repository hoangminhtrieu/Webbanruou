...
const Product = {
  ...
  findById(id) {
    ...
    // Parse JSON fields
    try {
      product.tasting_notes = JSON.parse(product.tasting_notes || "[]");
    } catch {
      product.tasting_notes = [];
    }
    try {
      product.food_pairing = JSON.parse(product.food_pairing || "[]");
    } catch {
  ...
  update(id, data) {
    ...
    return db
      .prepare(`UPDATE products SET ${fields} WHERE id = ?`)
      .run(...values, id);
  },
  ...
  addReview(productId, userId, { rating, comment }) {
    ...
    try {
      ...
      ).run(Math.round(avg.avg * 10) / 10, productId);
      return lastId(result);
    } catch (e) {

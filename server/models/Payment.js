...
const Payment = {
    // Tạo bản ghi payment
    create({ orderId, gateway, amount, transactionId = null, rawRequest = null }) {
        const db = getDB();
    ...
    updateStatus(id, { status, transactionId, gatewayRef, rawResponse }) {
        ...
        return db.prepare(`
            ...
            WHERE id = ?
        `).run(status, transactionId || null, gatewayRef || null, JSON.stringify(rawResponse), status, id);
    },
    ...
    markSuccess(orderId, { transactionId, gatewayRef, rawResponse } = {}) {
        ...
        db.prepare(`
            ...
            WHERE order_id = ?
        `).run(transactionId || null, gatewayRef || null, JSON.stringify(rawResponse || {}), orderId);

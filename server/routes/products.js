...
router.get('/', validateProductFilter, (req, res) => {
    ...
    if (req.query.min_price) { sql += ' AND price >= ?'; params.push(+req.query.min_price); }
    if (req.query.max_price) { sql += ' AND price <= ?'; params.push(+req.query.max_price); }
    if (req.query.min_score) { sql += ' AND score >= ?'; params.push(+req.query.min_score); }
    ...
    const total = db.prepare(sql.replace('SELECT *', 'SELECT COUNT(*) as count')).get(...params)?.count || 0;
    const products = db.prepare(sql + ' LIMIT ? OFFSET ?').all(...params, limit, offset);

    products.forEach(p => {
        try { p.tasting_notes = JSON.parse(p.tasting_notes); } catch { }
        try { p.food_pairing = JSON.parse(p.food_pairing); } catch { }
...
router.post('/', authMiddleware, adminMiddleware, (req, res) => {
    const db = getDB();
    const { name, region, subregion, type, grape, abv, volume, price, old_price, stock, score, vintage, badge, img, description, tasting_notes, food_pairing, tags } = req.body;
    if (!name || !region || !type || !price) return res.status(400).json({ error: 'Thiếu thông tin bắt buộc' });
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
  `).run(name, region, subregion, type, grape, abv, volume || '750ml', price, old_price || null, stock || 0, score || null, vintage || null, badge || null, img || null, description || null,
...
router.put('/:id', authMiddleware, adminMiddleware, (req, res) => {
    ...
    if (!p) return res.status(404).json({ error: 'Sản phẩm không tồn tại' });
    const fields = ['name', 'region', 'subregion', 'type', 'grape', 'abv', 'volume', 'price', 'old_price', 'stock', 'score', 'vintage', 'badge', 'img', 'description', 'is_active'];
    const updates = []; const vals = [];
    fields.forEach(f => { if (req.body[f] !== undefined) { updates.push(`${f} = ?`); vals.push(req.body[f]); } });
    if (!updates.length) return res.status(400).json({ error: 'Không có trường nào cần update' });

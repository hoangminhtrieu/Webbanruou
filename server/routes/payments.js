...
router.post('/vnpay/create', optionalAuth, (req, res) => {
    ...
    const expireDate = (() => {
        ...
        String(e.getHours()).padStart(2, '0'), String(e.getMinutes()).padStart(2, '0'), String(e.getSeconds()).padStart(2, '0')].join('');
    })();
...
router.get('/vnpay/return', (req, res) => {
    ...
    if (success && orderId) {
        ...
        db.prepare('UPDATE payments SET status = "success", transaction_id = ?, paid_at = datetime("now"), raw_response = ? WHERE order_id = ? AND gateway = "vnpay" AND status = "pending"')
...
router.post('/vnpay/ipn', (req, res) => {
    const { vnp_SecureHash, vnp_TxnRef, vnp_ResponseCode, vnp_Amount, ...params } = req.body;
    const sorted = sortObject(params);
    const signData = qs.stringify(sorted, { encode: false });
    const hmac = crypto.createHmac('sha512', cfg.vnpay.hashSecret);
    const signed = hmac.update(Buffer.from(signData, 'utf-8')).digest('hex');

    if (signed !== vnp_SecureHash) return res.json({ RspCode: '97', Message: 'Fail checksum' });

    const orderId = vnp_TxnRef?.split('_')[0];
    const db = getDB();
    const order = db.prepare('SELECT * FROM orders WHERE id = ?').get(+orderId);
    if (!order) return res.json({ RspCode: '01', Message: 'Order not found' });
    if (order.total !== Math.round(+vnp_Amount / 100)) return res.json({ RspCode: '04', Message: 'Invalid amount' });

    if (vnp_ResponseCode === '00') {
        db.prepare('UPDATE orders SET status = "confirmed", updated_at = datetime("now") WHERE id = ?').run(+orderId);
        db.prepare('UPDATE payments SET status = "success", paid_at = datetime("now") WHERE order_id = ? AND gateway = "vnpay" AND status = "pending"').run(+orderId);
    }
...
router.post('/momo/create', optionalAuth, async (req, res) => {
    ...
    const signature = crypto.createHmac('sha256', cfg.momo.secretKey).update(rawSignature).digest('hex');
    ...
    try {
        ...
        db.prepare(`INSERT INTO payments (order_id, gateway, gateway_ref, amount, status, raw_response) VALUES (?, 'momo', ?, ?, 'pending', ?)`)
            .run(order_id, orderId_m, order.total, JSON.stringify(result));

        res.json({ payment_url: result.payUrl, deep_link: result.deeplink, qr_code: result.qrCodeUrl });
    } catch (err) {
...
router.post('/momo/callback', (req, res) => {
    ...
    if (resultCode === 0) {
        ...
        db.prepare('UPDATE orders SET status = "confirmed", updated_at = datetime("now") WHERE id = ?').run(+orderNum);
        db.prepare('UPDATE payments SET status = "success", transaction_id = ?, paid_at = datetime("now") WHERE gateway_ref = ?').run(String(transId), orderId);
    }
    res.json({ message: 'OK' });
});
...
// ─── Stripe ──────────────────────────────────────────────────
let stripe;
try { stripe = require('stripe')(cfg.stripe.secretKey); } catch { }

router.post('/stripe/create-intent', optionalAuth, async (req, res) => {
    if (!stripe) return res.status(503).json({ error: 'Stripe chưa được cấu hình. Vui lòng thêm STRIPE_SECRET_KEY vào .env' });
    const { order_id } = req.body;
    const db = getDB();
    const order = db.prepare('SELECT * FROM orders WHERE id = ?').get(order_id);
    if (!order) return res.status(404).json({ error: 'Không tìm thấy đơn hàng' });
    ...
    try {
        ...
        db.prepare(`INSERT INTO payments (order_id, gateway, gateway_ref, amount, status) VALUES (?, 'stripe', ?, ?, 'pending')`)
            .run(order_id, paymentIntent.id, order.total);
        res.json({ client_secret: paymentIntent.client_secret, publishable_key: cfg.stripe.publishableKey });
    } catch (err) {
...
router.post('/stripe/webhook', require('express').raw({ type: 'application/json' }), async (req, res) => {
    ...
    let event;
    try {
        event = stripe.webhooks.constructEvent(req.body, req.headers['stripe-signature'], cfg.stripe.webhookSecret);
    ...
    if (event.type === 'payment_intent.succeeded') {
        ...
        if (orderId) {
            const db = getDB();
            db.prepare('UPDATE orders SET status = "confirmed", updated_at = datetime("now") WHERE id = ?').run(+orderId);
            db.prepare('UPDATE payments SET status = "success", transaction_id = ?, paid_at = datetime("now") WHERE gateway_ref = ?').run(pi.id, pi.id);
        }
    }
...
router.get('/status/:order_id', optionalAuth, (req, res) => {
    const db = getDB();
    const payment = db.prepare('SELECT gateway, status, paid_at, transaction_id FROM payments WHERE order_id = ? ORDER BY created_at DESC LIMIT 1').get(req.params.order_id);
    const order = db.prepare('SELECT status FROM orders WHERE id = ?').get(req.params.order_id);

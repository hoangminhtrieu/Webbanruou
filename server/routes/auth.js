...
try {
  getDB().exec("ALTER TABLE users ADD COLUMN username TEXT");
} catch (_) {}
...
router.post(
  ...
  (req, res) => {
    ...
    const user = db
      .prepare(
        "SELECT id, email, username, full_name, role, tier, points FROM users WHERE id = ?",
      )
      .get(lastId(result));
...
router.post("/login", [body("password").notEmpty()], (req, res) => {
  ...
  if (!user || !bcrypt.compareSync(password, user.password)) {
    return res
      .status(401)
      .json({ error: "Tên tài khoản hoặc mật khẩu không đúng" });
  }
  if (!user.is_active)
...
router.get("/me", authMiddleware, (req, res) => {
  ...
  const user = db
    .prepare(
      "SELECT id, email, username, full_name, phone, role, tier, points, created_at FROM users WHERE id = ?",
    )
    .get(req.user.id);
  if (!user)

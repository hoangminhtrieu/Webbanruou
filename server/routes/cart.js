...
router.post("/", authMiddleware, (req, res) => {
  ...
  const db = getDB();
  const product = db
    .prepare("SELECT id, stock FROM products WHERE id = ? AND is_active = 1")
    .get(product_id);
  if (!product)
  ...
  if (existing) {
    ...
    ).run(req.user.id, product_id, Math.min(qty, product.stock));
  }
...
router.put("/:product_id", authMiddleware, (req, res) => {
  ...
  if (qty === 0) {
    ...
    ).run(req.user.id, req.params.product_id);
  ...
  ).run(newQty, req.user.id, req.params.product_id);
...
router.post("/merge", authMiddleware, (req, res) => {
  ...
  for (const item of items) {
    ...
    if (existing) {
      ...
      db.prepare(
        "INSERT INTO cart_items (user_id, product_id, qty) VALUES (?, ?, ?)",
      ).run(req.user.id, pid, Math.min(qty, product.stock));
    }
    merged++;
  }
